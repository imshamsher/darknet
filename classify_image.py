...
def YOLO():
    ...
    weightPath = "darknet19.weights"
    metaPath = "cfg/imagenet1k.data"
    ...
    for filename in glob.iglob(IMAGE_PATH + '*.jpg', recursive=True):
        ...
        try:
            ...
            p = darknet.classify(netMain,metaMain,darknet_image)

            print(p)
            cv2.imshow('Demo', frame_resized)
            cv2.waitKey(0)
            cv2.imwrite(OUTPUT_PATH + '/' + os.path.basename(filename), frame_resized)
        except Exception as e:
            print(e)
            break
